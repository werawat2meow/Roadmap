...
export async function PUT(req, { params }) {
  try {
    ...
    const canEdit =
      hasPermission(user, "benefit.usage.edit") ||
      hasPermission(user, "benefit.usage.manage");

    if (!canEdit) {
      return NextResponse.json(
        { success: false, error: "ไม่มีสิทธิ์แก้ไขการใช้สิทธิ์" },
        { status: 403 }
      );
    }
    ...
    const usage_date = body?.usage_date || null;
    ...
    if (!usage_date) {
      return NextResponse.json(
        { success: false, error: "กรุณาเลือกวันที่ใช้สิทธิ์" },
        { status: 400 }
      );
    }
    ...
    console.error("BENEFIT_USAGE_PUT_ERROR:", error);

    return NextResponse.json(
      { success: false, error: "แก้ไขการใช้สิทธิ์ไม่สำเร็จ" },
      { status: 500 }
    );
  }
}
...
export async function DELETE(req, { params }) {
  try {
    ...
    const canDelete =
      hasPermission(user, "benefit.usage.delete") ||
      hasPermission(user, "benefit.usage.manage");

    if (!canDelete) {
      return NextResponse.json(
        { success: false, error: "ไม่มีสิทธิ์ลบการใช้สิทธิ์" },
        { status: 403 }
      );
    }
    ...
    return NextResponse.json({
      success: true,
      message: "ลบการใช้สิทธิ์สำเร็จ",
    });
  } catch (error) {
    console.error("BENEFIT_USAGE_DELETE_ERROR:", error);

    return NextResponse.json(
      { success: false, error: "ลบการใช้สิทธิ์ไม่สำเร็จ" },
      { status: 500 }
    );
  }
}