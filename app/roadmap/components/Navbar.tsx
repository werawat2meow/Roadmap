...
const menuItems = [
  ...
  { name: "Management", href: "/roadmap/executive", icon: Crown },
...
export default function Navbar({
  ...
  return (
    <div
      className={`relative flex flex-col bg-gray-900 text-white h-full transition-all duration-300 ease-in-out ${isCollapsed ? "w-20" : "w-64"}`}
    >
      <div className="flex items-center justify-between p-4 h-16 border-b border-gray-800">
        {!isCollapsed && (
          <h1 className="text-2xl font-bold text-white ml-2">Roadmap</h1>
        )}
        {isCollapsed && (
          <div className="hidden md:block p-2 font-bold bg-gray-700 rounded-md">
            R
          </div>
        )}
      </div>
              ...
              <li key={item.name} onClick={onLinkClick} className="relative">
                ...
                <Link href={item.href} title={isCollapsed ? item.name : ""}>
                  ...
                  >
                    ...
                    {!isCollapsed && (
                      <span className="ml-3 truncate grow">{item.name}</span>
                    )}
