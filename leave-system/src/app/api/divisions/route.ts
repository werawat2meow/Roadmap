...
export async function PUT(req: Request) {
  try {
    ...
    const updated = await prisma.division.update({
      where: { id },
      data: { name, ...(divisionId ? { departmentId: divisionId } : {}) },
    });
...
export async function DELETE(req: Request) {
  try {
    ...
    const unitCount = await prisma.unit.count({ where: { divisionId: id } });
    if (unitCount > 0 && !force) {
      return NextResponse.json(
        {
          error: "มีหน่วยอยู่ กรุณาลบหน่วยก่อน",
        },
        { status: 409 }
      );
    }
    ...
    if (force) {
      await prisma.$transaction([
        prisma.unit.deleteMany({ where: { divisionId: id } }),
