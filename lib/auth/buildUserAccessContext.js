...
/* =========================================================
   ...
   (in-flight dedupe: กันยิงซ้ำเมื่อมี request พร้อมกันสำหรับ user เดียวกัน)
========================================================= */
...
export function buildUserAccessContext(
  ...
  const cacheKey = userAccount?.id;

  if (!cacheKey) {
    return buildUserAccessContextInternal(
      userAccount
    );
  }

  if (pendingContexts.has(cacheKey)) {
    return pendingContexts.get(cacheKey);
  }

  const promise =
    buildUserAccessContextInternal(
      userAccount
    ).finally(() => {
      pendingContexts.delete(cacheKey);
    });

  pendingContexts.set(cacheKey, promise);

  return promise;
}

async function buildUserAccessContextInternal(
  userAccount
) {
