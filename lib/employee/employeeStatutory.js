...
export function hasEmployeeStatutoryPayload(
  ...
  /*
   * ห้ามใช้แค่ hasOwnProperty เพราะ Employee Payload
   * อาจมี social_security_no: null เพื่อ Legacy Compatibility
   * ซึ่งไม่ควรทำให้ Backend เปิด Statutory Validation
   */
  const hasTaxData =
    [
      "tax_identity_type",
...
function hasTaxWithholdingData(
  payload = {}
) {
  /*
   * Tax Identity อย่างเดียวไม่ถือว่า "บริษัทนำส่งภาษี"
   * เพราะ Identity อาจถูกแนบมากับ SSO-only Profile
   */
  return Boolean(
    cleanNullableText(
...
export function validateEmployeeStatutoryPayload(
  ...
  if (taxWithholdingEnabled) {
    ...
    if (!payload.tax_identity_type) {
      return "กรุณาระบุเลขประจำตัวสำหรับภาษี";
    }
    ...
    if (
      ...
      return "กรุณาเลือกสถานะผู้มีถิ่นที่อยู่ทางภาษี";
    }
