...
/* =========================================================
   EMPLOYEE CODE TYPE RESOLVER

   Priority:
   1. Executive position
   2. Part-time employment type
   3. Myanmar nationality
   4. Other foreign nationality (Non-B)
   5. Thai
========================================================= */

function normalizeEmployeeTypeToken(
  value
) {
  return String(value || "")
    .trim()
...
function isPartTimeEmploymentType(
  employmentType
) {
  if (!employmentType) {
    return false;
...
function resolveNationalityEmployeeType(
  nationality
) {
  if (!nationality) {
    return null;
  ...
  const name = [
    nationality.nationality_name_th,
...
async function resolveEmployeeCodeType({
  employee,
  position,
  ...
  if (employee.employment_type_id) {
    ...
    if (employmentTypeError) {
      return failure(
        "ไม่สามารถตรวจสอบประเภทการจ้างสำหรับสร้างรหัสพนักงานได้",
        500,
        employmentTypeError
      );
    }
  ...
  if (employee.nationality_id) {
    ...
    if (nationalityError) {
      return failure(
        "ไม่สามารถตรวจสอบสัญชาติสำหรับสร้างรหัสพนักงานได้",
        500,
        nationalityError
      );
    }

    const nationalityEmployeeType =
      resolveNationalityEmployeeType(
        nationality
      );

    if (nationalityEmployeeType) {
      return {
        success: true,
        employeeType:
          nationalityEmployeeType,
      };
    }
  }

  const fallbackEmployeeType =
    String(
      requestedEmployeeType || "thai"
    )
      .trim()
      .toLowerCase();
...
async function validateInitialCompensation({
  ...
  const isExecutivePosition =
    position?.is_executive === true;

  /*
    Salary Band เป็น Conditional Required ที่ Backend ด้วย

    - Executive: ไม่บังคับ Salary Band
    - Level ที่ไม่มี Band ที่เปิดใช้งานในวันที่เริ่มงาน: ไม่บังคับ
    - Level ทั่วไปที่มี Band ใช้งานได้: ต้องเลือก Band

    Backend เป็น Source of Truth เพื่อไม่ให้ Frontend/API คนละกฎ
  */
  if (
    !employee.position_level_band_id
  ) {
    let hasAvailableSalaryBand = false;
    ...
    if (
      ...
      if (availableBandsError) {
        return failure(
          "ไม่สามารถตรวจสอบ Salary Band ของระดับตำแหน่งได้",
          500,
          availableBandsError
        );
      }

      hasAvailableSalaryBand =
        (availableBands || []).some(
          (band) => {
            const effectiveDate =
              band?.effective_date || null;
            const expireDate =
              band?.expire_date || null;
...
export async function createEmployeeTransaction({
  ...
  try {
    ...
    */
    ...
    /* -----------------------------------------------------
       Resolve employee code type

       Backend ตรวจซ้ำอีกชั้น เพื่อไม่ให้ค่าจาก Frontend
       เช่น thai ถูกนำไปใช้กับตำแหน่ง Executive
    ----------------------------------------------------- */

    const employeeCodeTypeResult =
      await resolveEmployeeCodeType({
        employee,
        position:
          organizationResult.data
            ?.position || null,
    ...
    if (!employeeCodeTypeResult.success) {
      return employeeCodeTypeResult;
    }

    const resolvedEmployeeType =
      employeeCodeTypeResult
        .employeeType;
    ...
    if (
      ...
      const authEmail =
        ...
        `${String(
          ...
          .trim()
          .toLowerCase()}_${Date.now()}@local.user`;
